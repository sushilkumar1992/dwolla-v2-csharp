...
namespace DwollaFullFlow.Api.Services
{
    ...
    public class DwollaGateway : IDwollaGateway
    {
        ...
        {
            var headers = await BuildHeadersAsync();
            var fundingSourcesResponse = await _client.GetAsync<GetFundingSourcesResponse>(
                new Uri($"{_client.ApiBaseAddress}/customers/{customerId}/funding-sources?limit=50"),
                headers);

            EnsureSuccess(fundingSourcesResponse);

            var balanceSource = fundingSourcesResponse.Content.Embedded?.FundingSources?
                .FirstOrDefault(fs => fs.Type == "balance");

            if (balanceSource?.Links == null || !balanceSource.Links.TryGetValue("balance", out var balanceLink))
            {
                throw new DwollaApiException($"No balance funding source found for customer {customerId}.");
            }

            var response = await _client.GetAsync<BalanceResponse>(new Uri(balanceLink.Href), headers);
            EnsureSuccess(response);
            return response.Content;
        }
